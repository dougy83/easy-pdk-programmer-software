...
{
    // Serial routes through USB CDC with ARDUINO_USB_CDC_ON_BOOT
    return (bool)Serial;
}
...
{
    uint8_t tmp[] = { (uint8_t)rtype, (uint8_t)(len & 0xFF), (uint8_t)((len >> 8) & 0xFF) };
...
void FPDKUSB_SendDebug(const uint8_t* dat, const uint32_t len)
{
    ...
    _dbg_led_rx_off_tick = millis() + _dbg_led_on_time;
...
bool _FPDKUSB_HandleCmd(const FPDKPROTO_CMD cmd, const uint8_t* dat, const uint32_t len)
{
    ...
    case FPDKPROTO_CMD_BLANKCKIC:
    {
        ...
        FPDK_SetLed(FPDK_LED_IC, true);
        ic_id = FPDK_BlankCheckIC(ic_id, (FPDKICTYPE)type, vpp_cmd, vdd_cmd, vpp_read, vdd_read, addr_bits, data_bits, count, addr_exclude_first_instr, addr_exclude_start, addr_exclude_end);
        FPDK_SetLed(FPDK_LED_IC, false);
    ...
    case FPDKPROTO_CMD_ERASEIC:
    {
        ...
        FPDK_SetLed(FPDK_LED_IC, true);
        ic_id = FPDK_EraseIC(ic_id, (FPDKICTYPE)type, vpp_cmd, vdd_cmd, vpp_erase, vdd_erase, erase_clocks);
        FPDK_SetLed(FPDK_LED_IC, false);
    ...
    case FPDKPROTO_CMD_READIC:
    {
        ...
        FPDK_SetLed(FPDK_LED_IC, true);
        ic_id = FPDK_ReadIC(ic_id, (FPDKICTYPE)type, vpp_cmd, vdd_cmd, vpp_read, vdd_read, addr, addr_bits, &_ic_rw_buffer[data_offs], data_bits, count);
        FPDK_SetLed(FPDK_LED_IC, false);
    ...
    case FPDKPROTO_CMD_WRITEIC:
    {
        ...
        FPDK_SetLed(FPDK_LED_IC, true);
        ic_id = FPDK_WriteIC(ic_id, (FPDKICTYPE)type, vpp_cmd, vdd_cmd, vpp_write, vdd_write, addr, addr_bits, &_ic_rw_buffer[data_offs], data_bits, count, write_block_size, write_block_clock_groups, write_block_clocks_per_group);
        FPDK_SetLed(FPDK_LED_IC, false);
    ...
    case FPDKPROTO_CMD_VERIFYIC:
    {
        ...
        FPDK_SetLed(FPDK_LED_IC, true);
        ic_id = FPDK_VerifyIC(ic_id, (FPDKICTYPE)type, vpp_cmd, vdd_cmd, vpp_read, vdd_read, addr, addr_bits, &_ic_rw_buffer[data_offs], data_bits, count, addr_exclude_first_instr, addr_exclude_start, addr_exclude_end);
        FPDK_SetLed(FPDK_LED_IC, false);
    ...
    case FPDKPROTO_CMD_DBGDAT:
    {
        ...
        _dbg_led_tx_off_tick = millis() + _dbg_led_on_time;
...
{
    if (_dbg_led_rx_off_tick && (millis() > _dbg_led_rx_off_tick))
    {
        FPDK_SetLed(FPDK_LED_UART_RX, false);
        _dbg_led_rx_off_tick = 0;
    }

    if (_dbg_led_tx_off_tick && (millis() > _dbg_led_tx_off_tick))
    {
        FPDK_SetLed(FPDK_LED_UART_TX, false);
    ...
    if (!_FPDKUSB_HandleCmd((FPDKPROTO_CMD)_packetbuf[0], &_packetbuf[2], cmd_length))
        _FPDKUSB_SendError(0, 0);

    // Critical section: remove processed packet from buffer
    noInterrupts();
    if (_packetbufpos)
    {
